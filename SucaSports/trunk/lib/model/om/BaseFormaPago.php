  ...
  const PEER = 'FormaPagoPeer';
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 6; // 6 = FormaPagoPeer::NUM_COLUMNS - FormaPagoPeer::NUM_LAZY_LOAD_COLUMNS).
	 ...
	 * related CuentaCorrientes from storage. If this FormaPago is new, it will return
		...
		if ($this->collCuentaCorrientes === null) {
			if ($this->isNew()) {
			   $this->collCuentaCorrientes = array();
				...
				$criteria->add(CuentaCorrientePeer::ID_FORMA_PAGO, $this->id);
	 ...
	 */
	public function getCuentaCorrientesJoinCorredor($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(FormaPagoPeer::DATABASE_NAME);
		}
		...
		if ($this->collCuentaCorrientes === null) {
			if ($this->isNew()) {
				...
				$this->collCuentaCorrientes = CuentaCorrientePeer::doSelectJoinCorredor($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
