...
/**
 * Base class that represents a row from the 'etapa_carrera' table.
 *
 * 
 *
...
abstract class BaseEtapaCarrera extends BaseObject  implements Persistent {
  ...
  const PEER = 'EtapaCarreraPeer';

	/**
	 * The Peer class.
	 * Instance provides a convenient way of calling static methods on a class
	 * that calling code may not be able to identify.
	 * @var        EtapaCarreraPeer
	 */
	...
	/**
	 * The value for the numero_etapa field.
	 * @var        int
	 */
	...
	/**
	 * Initializes internal state of BaseEtapaCarrera object.
	 * @see        applyDefaults()
	 */
	...
	/**
	 * Get the [numero_etapa] column value.
	 * 
	 * @return     int
	 */
	 ...
	 * @return     EtapaCarrera The current object (for fluent API support)
	 */
	...
	/**
	 * Set the value of [numero_etapa] column.
	 * 
	 * @param      int $v new value
	 * @return     EtapaCarrera The current object (for fluent API support)
	 */
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->id_etapa = ($row[$startcol + 0] !== null) ? (int) $row[$startcol + 0] : null;
			$this->id_carrera = ($row[$startcol + 1] !== null) ? (int) $row[$startcol + 1] : null;
			$this->nombre = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
			$this->numero_etapa = ($row[$startcol + 3] !== null) ? (int) $row[$startcol + 3] : null;
			...
			return $startcol + 9; // 9 = EtapaCarreraPeer::NUM_COLUMNS - EtapaCarreraPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function ensureConsistency()
	{
		...
		if ($this->aCarrera !== null && $this->id_carrera !== $this->aCarrera->getId()) {
			$this->aCarrera = null;
		}
	} // ensureConsistency
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = EtapaCarreraPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
    ...
    foreach (sfMixer::getCallables('BaseEtapaCarrera:delete:pre') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseEtapaCarrera:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseEtapaCarrera:save:pre') as $callable)
    {
    ...
    if ($this->isModified() && !$this->isColumnModified(EtapaCarreraPeer::UPDATED_AT))
    {
      $this->setUpdatedAt(time());
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BaseEtapaCarrera:save:post') as $callable)
    {
      call_user_func($callable, $this, $con, $affectedRows);
    }

			EtapaCarreraPeer::addInstanceToPool($this);
			return $affectedRows;
		} catch (PropelException $e) {
			$con->rollBack();
			throw $e;
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					$this->setIdEtapa($pk);  //[IMV] update autoincrement primary key
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of EtapaCarrera (or compatible) type.
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setIdEtapa(NULL); // this is a auto-increment column, so set to default value
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     EtapaCarrera Clone of current object.
	 * @throws     PropelException
	 */
	...
	public function getCarrera(PropelPDO $con = null)
	{
		if ($this->aCarrera === null && ($this->id_carrera !== null)) {
			...
			$c->add(CarreraPeer::ID, $this->id_carrera);
			...
			/* The following can be used additionally to
			   ...
			   $this->aCarrera->addEtapaCarreras($this);
			 */
		}
  ...
  {
    if (!$callable = sfMixer::getCallable('BaseEtapaCarrera:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BaseEtapaCarrera::%s', $method));
    }
