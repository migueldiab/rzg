...
abstract class BaseInscripcion extends BaseObject  implements Persistent {
  ...
  const PEER = 'InscripcionPeer';
	...
	/**
	 * The value for the fecha_inscripcion field.
	 * @var        string
	 */
	...
	/**
	 * Initializes internal state of BaseInscripcion object.
	 * @see        applyDefaults()
	 */
	...
	/**
	 * Get the [optionally formatted] temporal [fecha_inscripcion] column value.
	 * 
	 *
	 * @param      string $format The date/time format string (either date()-style or strftime()-style).
	 *							If format is NULL, then the raw DateTime object will be returned.
	...
	public function getFechaInscripcion($format = 'Y-m-d')
	{
		...
		if ($this->fecha_inscripcion === '0000-00-00') {
			// while technically this is not a default value of NULL,
			// this seems to be closest in meaning.
			return null;
		} else {
			try {
				$dt = new DateTime($this->fecha_inscripcion);
			} catch (Exception $x) {
				throw new PropelException("Internally stored date/time/timestamp value could not be converted to DateTime: " . var_export($this->fecha_inscripcion, true), $x);
			}
		}
	 ...
	 * @return     Inscripcion The current object (for fluent API support)
	 */
	...
	/**
	 * Sets the value of [fecha_inscripcion] column to a normalized version of the date/time value specified.
	 * 
	 * @param      mixed $v string, integer (timestamp), or DateTime value.  Empty string will
	 *						be treated as NULL for temporal objects.
	 * @return     Inscripcion The current object (for fluent API support)
	...
	public function setFechaInscripcion($v)
	{
		...
		if ( $this->fecha_inscripcion !== null || $dt !== null ) {
			// (nested ifs are a little easier to read in this case)

			$currNorm = ($this->fecha_inscripcion !== null && $tmpDt = new DateTime($this->fecha_inscripcion)) ? $tmpDt->format('Y-m-d') : null;
			$newNorm = ($dt !== null) ? $dt->format('Y-m-d') : null;

			if ( ($currNorm !== $newNorm) // normalized values don't match 
					)
			{
				$this->fecha_inscripcion = ($dt ? $dt->format('Y-m-d') : null);
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->id_etapa = ($row[$startcol + 2] !== null) ? (int) $row[$startcol + 2] : null;
			$this->id_carrera = ($row[$startcol + 3] !== null) ? (int) $row[$startcol + 3] : null;
			...
			$this->fecha_inscripcion = ($row[$startcol + 8] !== null) ? (string) $row[$startcol + 8] : null;
			$this->firma_digital = ($row[$startcol + 9] !== null) ? (string) $row[$startcol + 9] : null;
			$this->id_categoria = ($row[$startcol + 10] !== null) ? (int) $row[$startcol + 10] : null;
			$this->resetModified();
			...
			return $startcol + 11; // 11 = InscripcionPeer::NUM_COLUMNS - InscripcionPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function ensureConsistency()
	{
		...
		if ($this->aCategoria !== null && $this->id_categoria !== $this->aCategoria->getId()) {
			$this->aCategoria = null;
		}
	} // ensureConsistency
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = InscripcionPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
    ...
    foreach (sfMixer::getCallables('BaseInscripcion:delete:pre') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseInscripcion:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseInscripcion:save:pre') as $callable)
    {
    ...
    if ($this->isModified() && !$this->isColumnModified(InscripcionPeer::UPDATED_AT))
    {
      $this->setUpdatedAt(time());
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BaseInscripcion:save:post') as $callable)
    {
      call_user_func($callable, $this, $con, $affectedRows);
    }

			InscripcionPeer::addInstanceToPool($this);
			return $affectedRows;
		} catch (PropelException $e) {
			$con->rollBack();
			throw $e;
	...
	public function getCorredor(PropelPDO $con = null)
	{
		if ($this->aCorredor === null && ($this->id_corredor !== null)) {
			...
			/* The following can be used additionally to
			   ...
			   $this->aCorredor->addInscripcions($this);
			 */
		}
	...
	public function getCategoria(PropelPDO $con = null)
	{
		if ($this->aCategoria === null && ($this->id_categoria !== null)) {
			...
			$c->add(CategoriaPeer::ID, $this->id_categoria);
			...
			/* The following can be used additionally to
			   ...
			   undesirable since it could result in an only partially populated collection
			   in the referenced object.
			   $this->aCategoria->addInscripcions($this);
			 */
		}
  ...
  {
    if (!$callable = sfMixer::getCallable('BaseInscripcion:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BaseInscripcion::%s', $method));
    }
