...
/**
 * Base class that represents a row from the 'portal' table.
 *
 * 
 *
...
abstract class BasePortal extends BaseObject  implements Persistent {
  ...
  const PEER = 'PortalPeer';
	...
	/**
	 * The value for the nombrepagina field.
	 * @var        string
	 */
	...
	/**
	 * Initializes internal state of BasePortal object.
	 * @see        applyDefaults()
	 */
	...
	/**
	 * Get the [nombrepagina] column value.
	 * 
	 * @return     string
	 */
	 ...
	 * @return     Portal The current object (for fluent API support)
	 */
	...
	/**
	 * Set the value of [nombrepagina] column.
	 * 
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->nombrepagina = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			...
			return $startcol + 5; // 5 = PortalPeer::NUM_COLUMNS - PortalPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = PortalPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
    ...
    foreach (sfMixer::getCallables('BasePortal:delete:pre') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BasePortal:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BasePortal:save:pre') as $callable)
    {
    ...
    if ($this->isModified() && !$this->isColumnModified(PortalPeer::UPDATED_AT))
    {
      $this->setUpdatedAt(time());
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BasePortal:save:post') as $callable)
    {
      call_user_func($callable, $this, $con, $affectedRows);
    }

			PortalPeer::addInstanceToPool($this);
			return $affectedRows;
		} catch (PropelException $e) {
			$con->rollBack();
			throw $e;
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of Portal (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     Portal Clone of current object.
	 * @throws     PropelException
	 */
  ...
  {
    if (!$callable = sfMixer::getCallable('BasePortal:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BasePortal::%s', $method));
    }
