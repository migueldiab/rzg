...
/**
 * Base class that represents a row from the 'chip' table.
 *
 * 
 *
...
abstract class BaseChip extends BaseObject  implements Persistent {
  ...
  const PEER = 'ChipPeer';
	...
	/**
	 * The value for the codigo_chip field.
	 * @var        string
	 */
	...
	/**
	 * Initializes internal state of BaseChip object.
	 * @see        applyDefaults()
	 */
	...
	/**
	 * Get the [codigo_chip] column value.
	 * 
	 * @return     string
	 */
	 ...
	 * @return     Chip The current object (for fluent API support)
	 */
	...
	/**
	 * Set the value of [codigo_chip] column.
	 * 
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->codigo_chip = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			...
			return $startcol + 7; // 7 = ChipPeer::NUM_COLUMNS - ChipPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = ChipPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
    ...
    foreach (sfMixer::getCallables('BaseChip:delete:pre') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseChip:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseChip:save:pre') as $callable)
    {
    ...
    if ($this->isModified() && !$this->isColumnModified(ChipPeer::UPDATED_AT))
    {
      $this->setUpdatedAt(time());
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BaseChip:save:post') as $callable)
    {
      call_user_func($callable, $this, $con, $affectedRows);
    }

			ChipPeer::addInstanceToPool($this);
			return $affectedRows;
		} catch (PropelException $e) {
			$con->rollBack();
			throw $e;
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of Chip (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     Chip Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 * Otherwise if this Chip has previously been saved, it will retrieve
	 * related Corredors from storage. If this Chip is new, it will return
		...
		if ($this->collCorredors === null) {
			if ($this->isNew()) {
			   $this->collCorredors = array();
				...
				$criteria->add(CorredorPeer::ID_CHIPS, $this->id);
	 ...
	 * Otherwise if this Chip is new, it will return
	 * an empty collection; or if this Chip has previously
	 * been saved, it will retrieve related Corredors from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in Chip.
	 */
	 ...
	 * actually need in Chip.
	 */
	public function getCorredorsJoinSociedadMedica($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(ChipPeer::DATABASE_NAME);
		}
		...
		if ($this->collCorredors === null) {
			if ($this->isNew()) {
				...
				$this->collCorredors = CorredorPeer::doSelectJoinSociedadMedica($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
  ...
  {
    if (!$callable = sfMixer::getCallable('BaseChip:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BaseChip::%s', $method));
    }
