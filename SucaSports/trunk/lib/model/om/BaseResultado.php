...
abstract class BaseResultado extends BaseObject  implements Persistent {
  ...
  const PEER = 'ResultadoPeer';
	...
	/**
	 * Initializes internal state of BaseResultado object.
	 * @see        applyDefaults()
	 */
	...
	/**
	 * Get the [optionally formatted] temporal [tiempo] column value.
	 * 
	 *
	 * @param      string $format The date/time format string (either date()-style or strftime()-style).
	 *							If format is NULL, then the raw DateTime object will be returned.
	...
	public function getTiempo($format = 'Y-m-d H:i:s')
	{
		...
		if ($this->tiempo === '0000-00-00 00:00:00') {
			// while technically this is not a default value of NULL,
			// this seems to be closest in meaning.
			return null;
		} else {
			try {
				$dt = new DateTime($this->tiempo);
			} catch (Exception $x) {
				throw new PropelException("Internally stored date/time/timestamp value could not be converted to DateTime: " . var_export($this->tiempo, true), $x);
			}
		}
	...
	/**
	 * Sets the value of [tiempo] column to a normalized version of the date/time value specified.
	 * 
	 * @param      mixed $v string, integer (timestamp), or DateTime value.  Empty string will
	 *						be treated as NULL for temporal objects.
	 * @return     Resultado The current object (for fluent API support)
	...
	public function setTiempo($v)
	{
		...
		if ( $this->tiempo !== null || $dt !== null ) {
			// (nested ifs are a little easier to read in this case)

			$currNorm = ($this->tiempo !== null && $tmpDt = new DateTime($this->tiempo)) ? $tmpDt->format('Y-m-d H:i:s') : null;
			$newNorm = ($dt !== null) ? $dt->format('Y-m-d H:i:s') : null;

			if ( ($currNorm !== $newNorm) // normalized values don't match 
					)
			{
				$this->tiempo = ($dt ? $dt->format('Y-m-d H:i:s') : null);
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->tiempo = ($row[$startcol + 4] !== null) ? (string) $row[$startcol + 4] : null;
			...
			return $startcol + 7; // 7 = ResultadoPeer::NUM_COLUMNS - ResultadoPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function getCorredor(PropelPDO $con = null)
	{
		if ($this->aCorredor === null && ($this->id_corredor !== null)) {
			...
			/* The following can be used additionally to
			   ...
			   $this->aCorredor->addResultados($this);
			 */
		}
