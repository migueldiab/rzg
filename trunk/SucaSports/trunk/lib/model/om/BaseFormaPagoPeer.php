...
abstract class BaseFormaPagoPeer {
	...
	private static $fieldNames = array (
		...
		BasePeer::TYPE_STUDLYPHPNAME => array ('id', 'nombre', 'createdAt', 'createdBy', 'updatedAt', 'updatedBy', ),
		BasePeer::TYPE_COLNAME => array (self::ID, self::NOMBRE, self::CREATED_AT, self::CREATED_BY, self::UPDATED_AT, self::UPDATED_BY, ),
	...
	private static $fieldKeys = array (
		...
		BasePeer::TYPE_STUDLYPHPNAME => array ('id' => 0, 'nombre' => 1, 'createdAt' => 2, 'createdBy' => 3, 'updatedAt' => 4, 'updatedBy' => 5, ),
		BasePeer::TYPE_COLNAME => array (self::ID => 0, self::NOMBRE => 1, self::CREATED_AT => 2, self::CREATED_BY => 3, self::UPDATED_AT => 4, self::UPDATED_BY => 5, ),
	...
	public static function getMapBuilder()
	{
		if (self::$mapBuilder === null) {
			self::$mapBuilder = new FormaPagoMapBuilder();
		}
    ...
    foreach (sfMixer::getCallables('BaseFormaPagoPeer:doCount:doCount') as $callable)
    {
      call_user_func($callable, 'BaseFormaPagoPeer', $criteria, $con);
    }
    ...
    foreach (sfMixer::getCallables('BaseFormaPagoPeer:doSelectStmt:doSelectStmt') as $callable)
    {
      call_user_func($callable, 'BaseFormaPagoPeer', $criteria, $con);
    }
	 ...
	 *
	 * @param      FormaPago $value A FormaPago object.
	 * @param      string $key (optional) key to use for instance map (for performance boost if key was already calculated externally).
	 */
	public static function addInstanceToPool(FormaPago $obj, $key = null)
	{
  ...
  static public function getUniqueColumnNames()
  {
	...
	/**
	 * Method perform an INSERT on the database, given a FormaPago or Criteria object.
	 *
    ...
    foreach (sfMixer::getCallables('BaseFormaPagoPeer:doInsert:pre') as $callable)
    {
      $ret = call_user_func($callable, 'BaseFormaPagoPeer', $values, $con);
      if (false !== $ret)
      {
        return $ret;
      }
		...
		if ($criteria->containsKey(FormaPagoPeer::ID) && $criteria->keyContainsValue(FormaPagoPeer::ID) ) {
			throw new PropelException('Cannot insert a value for auto-increment primary key ('.FormaPagoPeer::ID.')');
		}
    ...
    foreach (sfMixer::getCallables('BaseFormaPagoPeer:doInsert:post') as $callable)
    {
      call_user_func($callable, 'BaseFormaPagoPeer', $values, $con, $pk);
    }
	...
	/**
	 * Method perform an UPDATE on the database, given a FormaPago or Criteria object.
	 *
    ...
    foreach (sfMixer::getCallables('BaseFormaPagoPeer:doUpdate:pre') as $callable)
    {
      $ret = call_user_func($callable, 'BaseFormaPagoPeer', $values, $con);
      if (false !== $ret)
      {
    ...
    foreach (sfMixer::getCallables('BaseFormaPagoPeer:doUpdate:post') as $callable)
    {
      call_user_func($callable, 'BaseFormaPagoPeer', $values, $con, $ret);
    }
	...
	/**
	 * Method to DELETE all rows from the forma_pago table.
	 *
	 * @return     int The number of affected rows (if supported by underlying database driver).
	 */
	...
	/**
	 * Method perform a DELETE on the database, given a FormaPago or Criteria object OR a primary key value.
	 *
	 ...
	 */
	 public static function doDelete($values, PropelPDO $con = null)
	 {
		...
		} elseif ($values instanceof FormaPago) {
...
//
// NOTE: This static code cannot call methods on the FormaPagoPeer class, because it is not defined yet.
// If you need to use overridden methods, you can add this code to the bottom of the FormaPagoPeer class:
//
// Propel::getDatabaseMap(FormaPagoPeer::DATABASE_NAME)->addTableBuilder(FormaPagoPeer::TABLE_NAME, FormaPagoPeer::getMapBuilder());
//
// Doing so will effectively overwrite the registration below.

Propel::getDatabaseMap(BaseFormaPagoPeer::DATABASE_NAME)->addTableBuilder(BaseFormaPagoPeer::TABLE_NAME, BaseFormaPagoPeer::getMapBuilder());
