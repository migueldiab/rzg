...
/**
 * Base class that represents a row from the 'asociacion' table.
 *
 * 
 *
...
abstract class BaseAsociacion extends BaseObject  implements Persistent {
  ...
  const PEER = 'AsociacionPeer';
	...
	/**
	 * Initializes internal state of BaseAsociacion object.
	 * @see        applyDefaults()
	 */
	 ...
	 * @return     Asociacion The current object (for fluent API support)
	 */
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 9; // 9 = AsociacionPeer::NUM_COLUMNS - AsociacionPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = AsociacionPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
    ...
    foreach (sfMixer::getCallables('BaseAsociacion:delete:pre') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseAsociacion:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseAsociacion:save:pre') as $callable)
    {
    ...
    if ($this->isModified() && !$this->isColumnModified(AsociacionPeer::UPDATED_AT))
    {
      $this->setUpdatedAt(time());
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BaseAsociacion:save:post') as $callable)
    {
      call_user_func($callable, $this, $con, $affectedRows);
    }

			AsociacionPeer::addInstanceToPool($this);
			return $affectedRows;
		} catch (PropelException $e) {
			$con->rollBack();
			throw $e;
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of Asociacion (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     Asociacion Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 * Otherwise if this Asociacion has previously been saved, it will retrieve
	 * related AsociacionCorredors from storage. If this Asociacion is new, it will return
		...
		if ($this->collAsociacionCorredors === null) {
			if ($this->isNew()) {
			   $this->collAsociacionCorredors = array();
				...
				$criteria->add(AsociacionCorredorPeer::ID_ASOCIACION, $this->id);
	 ...
	 * Otherwise if this Asociacion is new, it will return
	 * an empty collection; or if this Asociacion has previously
	 * been saved, it will retrieve related AsociacionCorredors from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in Asociacion.
	 */
	public function getAsociacionCorredorsJoinCorredor($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(AsociacionPeer::DATABASE_NAME);
		}
		...
		if ($this->collAsociacionCorredors === null) {
			if ($this->isNew()) {
				...
				$this->collAsociacionCorredors = AsociacionCorredorPeer::doSelectJoinCorredor($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
  ...
  {
    if (!$callable = sfMixer::getCallable('BaseAsociacion:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BaseAsociacion::%s', $method));
    }
