...
abstract class BaseCarrera extends BaseObject  implements Persistent {
  ...
  const PEER = 'CarreraPeer';
	...
	/**
	 * @var        array EtapaCarrera[] Collection to store aggregation of EtapaCarrera objects.
	 */
	protected $collEtapaCarreras;

	/**
	 * @var        Criteria The criteria used to select the current contents of collEtapaCarreras.
	 */
	private $lastEtapaCarreraCriteria = null;
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 8; // 8 = CarreraPeer::NUM_COLUMNS - CarreraPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		if ($deep) {  // also de-associate any related objects?
			...
			$this->collEtapaCarreras = null;
			$this->lastEtapaCarreraCriteria = null;
	 ...
	 * related CategoriaCarreras from storage. If this Carrera is new, it will return
		...
		if ($this->collCategoriaCarreras === null) {
			if ($this->isNew()) {
			   $this->collCategoriaCarreras = array();
				...
				$criteria->add(CategoriaCarreraPeer::ID_CARRERA, $this->id);
	 ...
	 */
	public function getCategoriaCarrerasJoinCategoria($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(CarreraPeer::DATABASE_NAME);
		}
		...
		if ($this->collCategoriaCarreras === null) {
			if ($this->isNew()) {
				...
				$this->collCategoriaCarreras = CategoriaCarreraPeer::doSelectJoinCategoria($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
	...
	/**
	 * Clears out the collEtapaCarreras collection (array).
	 *
	 * This does not modify the database; however, it will remove any associated objects, causing
	 * them to be refetched by subsequent calls to accessor method.
	 *
	 * @return     void
	 * @see        addEtapaCarreras()
	 */
	public function clearEtapaCarreras()
	{
		$this->collEtapaCarreras = null; // important to set this to NULL since that means it is uninitialized
	}

	/**
	 * Initializes the collEtapaCarreras collection (array).
	 *
	 * By default this just sets the collEtapaCarreras collection to an empty array (like clearcollEtapaCarreras());
	 * however, you may wish to override this method in your stub class to provide setting appropriate
	 * to your application -- for example, setting the initial array to the values stored in database.
	 *
	 * @return     void
	...
	/**
	 * Gets an array of EtapaCarrera objects which contain a foreign key that references this object.
	 *
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this Carrera has previously been saved, it will retrieve
	 * related EtapaCarreras from storage. If this Carrera is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
	 * @return     array EtapaCarrera[]
	 * @throws     PropelException
	 */
	public function getEtapaCarreras($criteria = null, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(CarreraPeer::DATABASE_NAME);
		}
		...
		if ($this->collEtapaCarreras === null) {
			if ($this->isNew()) {
			   $this->collEtapaCarreras = array();
				...
				$criteria->add(EtapaCarreraPeer::ID_CARRERA, $this->id);
	...
	/**
	 * Returns the number of related EtapaCarrera objects.
	 *
	 * @param      Criteria $criteria
	 * @param      boolean $distinct
	 * @param      PropelPDO $con
	 * @return     int Count of related EtapaCarrera objects.
	 * @throws     PropelException
	 */
	public function countEtapaCarreras(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(CarreraPeer::DATABASE_NAME);
		} else {
		...
		if ($this->collEtapaCarreras === null) {
			if ($this->isNew()) {
				...
				$count = EtapaCarreraPeer::doCount($criteria, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				if (!isset($this->lastEtapaCarreraCriteria) || !$this->lastEtapaCarreraCriteria->equals($criteria)) {
					...
					$count = count($this->collEtapaCarreras);
				}
			} else {
				$count = count($this->collEtapaCarreras);
			}
	...
	/**
	 * Method called to associate a EtapaCarrera object to this object
	 * through the EtapaCarrera foreign key attribute.
	 *
	 * @param      EtapaCarrera $l EtapaCarrera
	 * @return     void
	 * @throws     PropelException
	 */
	...
	{
		if ($this->collEtapaCarreras === null) {
			$this->initEtapaCarreras();
		}
		if (!in_array($l, $this->collEtapaCarreras, true)) { // only add it if the **same** object is not already associated
			array_push($this->collEtapaCarreras, $l);
			$l->setCarrera($this);
		}
	}
	...
	public function clearAllReferences($deep = false)
	{
		if ($deep) {
			...
			if ($this->collEtapaCarreras) {
				foreach ((array) $this->collEtapaCarreras as $o) {
					$o->clearAllReferences($deep);
				}
			}
		} // if ($deep)
