...
/**
 * Base class that represents a row from the 'equipamiento' table.
 *
 * 
 *
...
abstract class BaseEquipamiento extends BaseObject  implements Persistent {
  ...
  const PEER = 'EquipamientoPeer';

	/**
	 * The Peer class.
	 * Instance provides a convenient way of calling static methods on a class
	 * that calling code may not be able to identify.
	 * @var        EquipamientoPeer
	 */
	...
	/**
	 * Initializes internal state of BaseEquipamiento object.
	 * @see        applyDefaults()
	 */
	 ...
	 * @return     Equipamiento The current object (for fluent API support)
	 */
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->marca = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			...
			return $startcol + 7; // 7 = EquipamientoPeer::NUM_COLUMNS - EquipamientoPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function ensureConsistency()
	{
		...
		if ($this->aTipoEquipamiento !== null && $this->id_tipo_equipamiento !== $this->aTipoEquipamiento->getId()) {
			$this->aTipoEquipamiento = null;
		}
	} // ensureConsistency
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = EquipamientoPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
    ...
    foreach (sfMixer::getCallables('BaseEquipamiento:delete:pre') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseEquipamiento:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseEquipamiento:save:pre') as $callable)
    {
    ...
    if ($this->isModified() && !$this->isColumnModified(EquipamientoPeer::UPDATED_AT))
    {
      $this->setUpdatedAt(time());
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BaseEquipamiento:save:post') as $callable)
    {
      call_user_func($callable, $this, $con, $affectedRows);
    }

			EquipamientoPeer::addInstanceToPool($this);
			return $affectedRows;
		} catch (PropelException $e) {
			$con->rollBack();
			throw $e;
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->aTipoEquipamiento !== null) {
				if ($this->aTipoEquipamiento->isModified() || $this->aTipoEquipamiento->isNew()) {
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of Equipamiento (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     Equipamiento Clone of current object.
	 * @throws     PropelException
	 */
	...
	/**
	 * Declares an association between this object and a TipoEquipamiento object.
	 *
	 * @param      TipoEquipamiento $v
	 * @return     Equipamiento The current object (for fluent API support)
	 * @throws     PropelException
	 */
	public function setTipoEquipamiento(TipoEquipamiento $v = null)
	{
		if ($v === null) {
		...
		// If this object has already been added to the TipoEquipamiento object, it will not be re-added.
		if ($v !== null) {
			$v->addEquipamiento($this);
		}
	...
	/**
	 * Get the associated TipoEquipamiento object
	 *
	 * @param      PropelPDO Optional Connection object.
	 * @return     TipoEquipamiento The associated TipoEquipamiento object.
	 * @throws     PropelException
	 */
	public function getTipoEquipamiento(PropelPDO $con = null)
	{
		if ($this->aTipoEquipamiento === null && ($this->id_tipo_equipamiento !== null)) {
			$c = new Criteria(TipoEquipamientoPeer::DATABASE_NAME);
			$c->add(TipoEquipamientoPeer::ID, $this->id_tipo_equipamiento);
			$this->aTipoEquipamiento = TipoEquipamientoPeer::doSelectOne($c, $con);
			/* The following can be used additionally to
			   guarantee the related object contains a reference
			   to this object.  This level of coupling may, however, be
			   undesirable since it could result in an only partially populated collection
			   in the referenced object.
			   $this->aTipoEquipamiento->addEquipamientos($this);
			 */
		}
	 ...
	 * Otherwise if this Equipamiento has previously been saved, it will retrieve
	 * related CorredorEquipamientos from storage. If this Equipamiento is new, it will return
		...
		if ($this->collCorredorEquipamientos === null) {
			if ($this->isNew()) {
			   $this->collCorredorEquipamientos = array();
				...
				$criteria->add(CorredorEquipamientoPeer::ID_EQUIPAMIENTO, $this->id);
	 ...
	 * Otherwise if this Equipamiento is new, it will return
	 * an empty collection; or if this Equipamiento has previously
	 * been saved, it will retrieve related CorredorEquipamientos from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in Equipamiento.
	 */
	public function getCorredorEquipamientosJoinCorredor($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(EquipamientoPeer::DATABASE_NAME);
		}
		...
		if ($this->collCorredorEquipamientos === null) {
			if ($this->isNew()) {
				...
				$this->collCorredorEquipamientos = CorredorEquipamientoPeer::doSelectJoinCorredor($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this Equipamiento has previously been saved, it will retrieve
	 * related Inventarios from storage. If this Equipamiento is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
		...
		if ($this->collInventarios === null) {
			if ($this->isNew()) {
			   $this->collInventarios = array();
				...
				$criteria->add(InventarioPeer::ID_TIPO_EQUIPAMIENTO, $this->id);
	 ...
	 * actually need in Equipamiento.
	 */
	public function getInventariosJoinEstado($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(EquipamientoPeer::DATABASE_NAME);
		}
		...
		if ($this->collInventarios === null) {
			if ($this->isNew()) {
				...
				$this->collInventarios = InventarioPeer::doSelectJoinEstado($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
  ...
  {
    if (!$callable = sfMixer::getCallable('BaseEquipamiento:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BaseEquipamiento::%s', $method));
    }
