...
abstract class BaseGrupo extends BaseObject  implements Persistent {
  ...
  const PEER = 'GrupoPeer';
	...
	/**
	 * @var        array Permiso[] Collection to store aggregation of Permiso objects.
	 */
	protected $collPermisos;

	/**
	 * @var        Criteria The criteria used to select the current contents of collPermisos.
	 */
	private $lastPermisoCriteria = null;
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 6; // 6 = GrupoPeer::NUM_COLUMNS - GrupoPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	/**
	 * Clears out the collPermisos collection (array).
	 *
	...
	/**
	 * Initializes the collPermisos collection (array).
	 *
	 ...
	 * related Permisos from storage. If this Grupo is new, it will return
		...
		if ($this->collPermisos === null) {
			if ($this->isNew()) {
			   $this->collPermisos = array();
				...
				$criteria->add(PermisoPeer::GRUPOS_ID, $this->id);
	...
	public function countPermisos(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		...
		if ($this->collPermisos === null) {
			if ($this->isNew()) {
				...
				$count = PermisoPeer::doCount($criteria, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
	...
	public function addPermiso(Permiso $l)
	{
		...
		if (!in_array($l, $this->collPermisos, true)) { // only add it if the **same** object is not already associated
			array_push($this->collPermisos, $l);
			$l->setGrupo($this);
		}
	}
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this Grupo has previously been saved, it will retrieve
	 * related Usuarios from storage. If this Grupo is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
		...
		if ($this->collUsuarios === null) {
			if ($this->isNew()) {
			   $this->collUsuarios = array();
				...
				$criteria->add(UsuarioPeer::ID_GRUPO, $this->id);
	 ...
	 */
	public function getUsuariosJoinCorredor($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(GrupoPeer::DATABASE_NAME);
		}
		...
		if ($this->collUsuarios === null) {
			if ($this->isNew()) {
				...
				$this->collUsuarios = UsuarioPeer::doSelectJoinCorredor($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
	...
	public function clearAllReferences($deep = false)
	{
		if ($deep) {
			if ($this->collPermisos) {
				foreach ((array) $this->collPermisos as $o) {
