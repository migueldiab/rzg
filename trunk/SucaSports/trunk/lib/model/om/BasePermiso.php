...
abstract class BasePermiso extends BaseObject  implements Persistent {
  ...
  const PEER = 'PermisoPeer';
	...
	/**
	 * The value for the grupos_id field.
	 * @var        int
	 */
	protected $grupos_id;
	...
	/**
	 * Initializes internal state of BasePermiso object.
	 * @see        applyDefaults()
	 */
	...
	/**
	 * Get the [grupos_id] column value.
	 * 
	 * @return     int
	 */
	...
	/**
	 * Set the value of [grupos_id] column.
	 * 
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->permiso = ($row[$startcol + 0] !== null) ? (string) $row[$startcol + 0] : null;
			$this->grupos_id = ($row[$startcol + 1] !== null) ? (int) $row[$startcol + 1] : null;
			...
			return $startcol + 4; // 4 = PermisoPeer::NUM_COLUMNS - PermisoPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function ensureConsistency()
	{
		...
		if ($this->aGrupo !== null && $this->grupos_id !== $this->aGrupo->getId()) {
			$this->aGrupo = null;
		}
	} // ensureConsistency
    ...
    foreach (sfMixer::getCallables('BasePermiso:delete:pre') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BasePermiso:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BasePermiso:save:pre') as $callable)
    {
    ...
    if ($this->isModified() && !$this->isColumnModified(PermisoPeer::UPDATED_AT))
    {
      $this->setUpdatedAt(time());
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BasePermiso:save:post') as $callable)
    {
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setGruposId($this->grupos_id);
	...
	public function getGrupo(PropelPDO $con = null)
	{
		if ($this->aGrupo === null && ($this->grupos_id !== null)) {
			...
			$c->add(GrupoPeer::ID, $this->grupos_id);
  ...
  {
    if (!$callable = sfMixer::getCallable('BasePermiso:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BasePermiso::%s', $method));
    }
